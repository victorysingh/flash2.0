"""
PacketPath brute-force pathfinding module.

This module implements a recursive, brute-force algorithm to find all possible
paths between two nodes in a graph. It is used to demonstrate the computational
inefficiency of exhaustive search compared to optimized algorithms like BFS for
...
def _find_all_paths_recursive(graph, current_node, target_node, current_path, all_paths):
    """
    Recursively explore paths from the current node.

    This is a helper function that performs the core DFS-like exploration to
    find every possible simple path (no repeated nodes).

    Args:
        graph (Graph): The graph to search.
        current_node (str): The node to start exploration from.
        target_node (str): The destination node.
        current_path (list[str]): The path taken so far.
        all_paths (list[list[str]]): A list to store all found paths.
    """
...
def find_shortest_path_brute_force(graph, start_node, target_node):
    """
    Find the shortest path by exploring all possible paths.

    This function initiates the recursive search, measures its execution time,
    and then identifies the shortest path from all the paths found.
    ...
    Returns:
        tuple[list[str] | None, float]: A tuple containing the shortest path
        (or None if no path is found) and the execution time in seconds.
    """
...
def display_brute_force_results(path, exec_time):
    """Display the results of the brute-force search."""
...
def main():
    """
    Run a demo comparing Brute-Force pathfinding with BFS.
    """
